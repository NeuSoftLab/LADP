...
class FDS(nn.Module):
    ...
    def update_running_stats(self, features, labels, epoch):
        if epoch < self.epoch:
            return
        print(self.feature_dim, features.shape[1])
        assert self.feature_dim == features.shape[1], "Input feature dimension is not aligned!"
        assert features.shape[0] == labels.shape[0], "Dimensions of features and labels are not aligned!"
